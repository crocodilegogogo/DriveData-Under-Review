...
def get_sep_flags(classifier_name, dataset_name, sep_flags):
    ...
    if classifier_name in ['IF_ConvTransformer_torch', \
                           'TSF_torch'] and \
        dataset_name in ['HAPT', 'HHAR', 'Opportunity', 'DSADS']:
        cur_sep_flag = True
        sep_flags.append('Sep')
    # elif classifier_name in ['DeepSense_torch'] and dataset_name in ['HAPT']:
        # cur_sep_flag = True
        # sep_flags.append('Sep')
    else:
        cur_sep_flag = False
...
def STFT_transform(X_data, y_data, STFT_intervals, test_split):
    ...
    if X_data.shape[0] < test_split:
        batch_size_cur = X_data.shape[0]
    else:
        batch_size_cur = X_data.shape[0] // test_split
...
def STFT_precessing(X_train, y_train, classifier_name, STFT_intervals, POS_NUM, test_split, aug_method='others'):
    if classifier_name == 'DeepSense_torch' and aug_method != 'HMGAN' and aug_method != 'FCGAN':
...
def create_cuda_classifier(dataset_name, classifier_name, INPUT_CHANNEL, POS_NUM, data_length,
    ...
    classifier, classifier_func = create_classifier(dataset_name, classifier_name, INPUT_CHANNEL, POS_NUM,
                                                    ...
                                                    nb_classes, STFT_intervals, BATCH_SIZE, args, AUGMENT_METHODS)
...
def training_process(logger, subject_id, X_tr, X_val, X_test, Y_tr, Y_val,
                     ...
                     POS_NUM, AUGMENT_METHODS, log_training_duration,
    ...
    if flag_output_directory_models is not None:
        ...
        print('----AUGMENT_METHODS: '+ "+".join(AUGMENT_METHODS))
        history, per_training_duration, log_training_duration = classifier_func.train_op(classifier, AUGMENT_METHODS, EPOCH, BATCH_SIZE, LR, POS_NUM,
...
def get_test_loss_acc(net, loss_function, x_data, y_data, test_split=1, test_flag=True):
    if net.aug_methods[0] == 'DriveData':
        net.set_augmenting(False)
    ...
    for step, (x,y) in enumerate(data_loader):
        with torch.no_grad():
            if args.INFERENCE_DEVICE == 'TEST_CUDA':
                # if net.aug_methods[0] == 'HMGAN' or net.aug_methods[0] == 'FCGAN':
                #     x = torch.split(x.squeeze(1).cuda(), 3, dim=1)
                # else:
                #     x = x.cuda()
                x = x.cuda()
                y = y.cuda()
            output_bc = net(x, test_flag=test_flag)[0]
...
def model_predict(net, x_data, y_data, test_split=1, test_flag=True):
    ...
    for step, (x,y) in enumerate(data_loader):
        with torch.no_grad():
            if args.INFERENCE_DEVICE == 'TEST_CUDA':
                ...
                #     x = x.cuda()
                x = x.cuda()
            output_bc, attn_bc = net(x, test_flag=test_flag)
    ...
    return output.cpu().data.numpy(), IMU_attentions # IMU_attentions.cpu().data.numpy()
...
def logging_settings(classifier_name, CUR_DIR, dataset_name, AUGMENT_METHODS):
    
    # Logging settings
    # EXEC_TIME = classifier_name + "+" + "+".join(AUGMENT_METHODS) + "-" + datetime.now().strftime("%Y%m%d-%H%M%S")
    EXEC_TIME = classifier_name + "-" + datetime.now().strftime("%Y%m%d-%H%M%S")
    LOG_DIR = os.path.join(CUR_DIR, f"logs", dataset_name, classifier_name+"+"+"+".join(AUGMENT_METHODS), f"{EXEC_TIME}")
    MODEL_DIR = os.path.join(CUR_DIR, f"saved_models", dataset_name, classifier_name+"+"+"+".join(AUGMENT_METHODS))
...
def log_final_test_results(logger, log_training_duration, scores, label2act, nb_classes, ALL_SUBJECTS_ID,
                           start1, end1, X_tr, y_train, y_test, AUGMENT_METHODS_LIST, augment_methods, time2, dataset_name, classifier_name, 
                           MODELS_COMP_LOG_DIR, CLASSIFIERS, classifier, classifier_parameter, fileHandler):
    ...
    save_classifiers_comparison(MODELS_COMP_LOG_DIR, CLASSIFIERS, classifier_name,
                                ...
                                len(ALL_SUBJECTS_ID), AUGMENT_METHODS_LIST, augment_methods, flops, classifier_parameter['Trainable'])
...
def log_flops(logger, network, train_x):
    ...
    if args.INFERENCE_DEVICE == 'TEST_CUDA':
        inputs = inputs.cuda()
    # if network.aug_methods[0] == 'HMGAN' or network.aug_methods[0] == 'FCGAN':
    #     inputs = torch.split(inputs.squeeze(1), 3, dim=1)
...
def save_classifiers_comparison(MODELS_COMP_LOG_DIR, CLASSIFIERS, classifier_name,
                                scores, ALL_SUBJECTS_ID, preprocess_time, inference_time,
                                SUBJECT_NUM, AUGMENT_METHODS_LIST, augment_methods, flops, paras):
    ...
    averaged_score_pd = pd.DataFrame(data    = np.zeros((len(index_metrics), 1),dtype=np.str_),
                                     index   = index_metrics,
                                     columns = [classifier_name+'+'+'+'.join(augment_methods)])
    ...
    if classifier_name == CLASSIFIERS[0] and augment_methods == AUGMENT_METHODS_LIST[0]:
