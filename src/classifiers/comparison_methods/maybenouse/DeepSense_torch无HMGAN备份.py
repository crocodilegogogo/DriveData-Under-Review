...
class DeepSense(nn.Module):
    def __init__(self, input_2Dfeature_channel, input_channel, POS_NUM, kernel_size,
                 ...
                 hidden_size, drop_rate, drop_rate_gru, num_class, datasetname, args):
    ...
    def new(self):
        network_new = DeepSense(self.input_2Dfeature_channel, self.input_channel, self.POS_NUM, self.kernel_size,
                                ...
                                self.num_class, self.datasetname, self.args)
        return network_new
    ...
    def forward(self, x, aug_methods=['DaDa'], test_flag=False):
        
        if self.augmenting and aug_methods[0]=='DaDa':
...
def train_op(network, aug_methods, EPOCH, BATCH_SIZE, LR, POS_NUM,
    ...
    for epoch in range (EPOCH):
        ...
        if aug_methods[0] != 'DaDa':
            ...
            for step, (x,y) in enumerate(train_loader):
                ...
                if 'Mixup' in aug_methods:
                    ...
                    output = network(batch_x, aug_methods=aug_methods)
        ...
        loss_train, accuracy_train, _ = get_test_loss_acc(network, loss_function, train_x, train_y, aug_methods, args.test_split)
        loss_validation, accuracy_validation, macro_f1_val = get_test_loss_acc(network, loss_function, val_x, val_y, aug_methods, args.test_split)
        loss_test, accuracy_test, macro_f1_test = get_test_loss_acc(network, loss_function, X_test, y_test, aug_methods, args.test_split)
        network.train()
