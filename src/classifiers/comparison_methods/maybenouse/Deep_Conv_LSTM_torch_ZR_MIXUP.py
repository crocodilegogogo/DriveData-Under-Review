...
class Deep_Conv_LSTM(nn.Module):
    def __init__(self, input_2Dfeature_channel, input_channel, kernel_size,
        ...
        self.build_prior = build_prior(num_class, feature_channel)
        ...
        for n, v in self.named_parameters():
            if n != 'conv_sigma.0.weight' and n != 'conv_sigma.0.bias' and n != 'build_prior.prior_mu' and n!='build_prior.prior_logvar':
    ...
    def data_generator(self, x, y_a, y_b, lam, test_flag):
        mu          = self.conv_mu(x)
        if test_flag == False:
            index_a = F.one_hot(torch.tensor(y_a),self.num_class).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
            index_b = F.one_hot(torch.tensor(y_b),self.num_class).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
            index   = lam * index_a + (1-lam) * index_b
            # sigma   = torch.exp(self.conv_sigma(x))
            # z       = mu + sigma * self.Normal.sample(mu.shape).to(x.device)
            sigma   = self.conv_sigma(x)
            z       = mu + torch.exp(0.5*sigma) * self.Normal.sample(mu.shape).to(x.device)
            
            p = self.build_prior(index.cuda())
            p_mu,p_sigma = p['mu'], p['logvar']
            # mu_loss      = (((mu-p_mu)**2)/(p_sigma.exp()+1e-6)).mean(dim=1,keepdim=True).mean(dim=2,keepdim=True).mean(dim=3,keepdim=True)
            # sigma_loss   = ((sigma-p_sigma).exp() - (sigma-p_sigma) -1 ).mean(dim=1,keepdim=True).mean(dim=2,keepdim=True).mean(dim=3,keepdim=True)
            mu_loss      = (((mu.mean(dim=2,keepdim=True).mean(dim=3,keepdim=True)-p_mu)**2)/(p_sigma.exp()+1e-6)).mean(dim=1,keepdim=True) #.mean(dim=2,keepdim=True).mean(dim=3,keepdim=True)
            sigma_loss   = ((sigma.mean(dim=2,keepdim=True).mean(dim=3,keepdim=True)-p_sigma).exp() - (sigma.mean(dim=2,keepdim=True).mean(dim=3,keepdim=True)-p_sigma) -1).mean(dim=1,keepdim=True) #.mean(dim=2,keepdim=True).mean(dim=3,keepdim=True)
            kl_loss      = (0.5*(mu_loss + sigma_loss)).sum()
    ...
    def forward(self, x, test_flag=False, y_a=0, y_b=0, lam=0, epoch=0):
        ...
        if self.aug_methods[0]=='DaDa':
            # index = F.one_hot(torch.tensor(int(y)),self.num_class) #.permute(0,4,1,2,3) #b*1*h*w->b*1*h*w*k->b*k*1*h*w ,将标签变为one-hot张量
            x, kl_loss = self.data_generator(x, y_a, y_b, lam, test_flag)
        else:
            kl_loss = 0
...
class build_prior(nn.Module):
    def __init__(self, num_class, feature_channel):
        super(build_prior, self).__init__()

        self.latent_dim = feature_channel #隐变量维度
        #生成每个类别的先验均值和方差p,隐变量维度latent_dim要和q一致
        prior_mu = torch.Tensor(1,num_class,self.latent_dim,1,1)#1*k*latent_dim*1*1
        prior_mu.normal_(0, math.sqrt(2. / self.latent_dim))
        prior_mu.requires_grad = True
        self.prior_mu = torch.nn.parameter.Parameter(data=prior_mu, requires_grad=True)

        prior_logvar = torch.Tensor(1,num_class,self.latent_dim,1,1)
        prior_logvar.normal_(0, math.sqrt(2. / self.latent_dim))
        prior_logvar.requires_grad = True
        self.prior_logvar = torch.nn.parameter.Parameter(data=prior_logvar, requires_grad=True)

    def build_priormu(self,index):
        #根据标签，填充每个像素的先验均值方差，得到先验均值、方差张量p，对齐后验均值、方差张量q维度,便于后续计算
        b = index.size(0)
        prior_mu = self.prior_mu.repeat(b,1,1,1,1)#b*k*latent*1*1
        prior_mu = (prior_mu*index).sum(dim=1)#b*k*latent*h*w -> b*latent*h*w 
        prior_logvar = self.prior_logvar.repeat(b,1,1,1,1)#b*k*latent*1*1
        prior_logvar = (prior_logvar*index).sum(dim=1)#b*k*latent*h*w -> b*latent*h*w 
        return prior_mu, prior_logvar

    def forward(self, labels):
        prior_mu, prior_logvar = self.build_priormu(labels)#根据标签
        p = {}
        p['mu'] = prior_mu
        p['logvar'] = prior_logvar
        p['index'] = labels
        return p
        ...
        for epoch in range (EPOCH):
            ...
            if aug_methods[0] != 'DaDa':
                ...
                for step, (x,y) in enumerate(train_loader):
                    ...
                    # output_bc, kl_loss = network(x, y=y, epoch=epoch)
                    # # cal the sum of pre loss per batch 
                    ...
                    output_bc, kl_loss = network(mixed_x, y_a=y_a, y_b=y_b, lam=lam, epoch=epoch)
