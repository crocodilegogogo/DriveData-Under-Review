...
class Deep_Conv_LSTM(nn.Module):
    ...
    def data_generator(self, x, y, test_flag):
        mu          = self.conv_mu(x)
        if test_flag == False:
            index   = F.one_hot(y,self.num_class).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
            sigma   = self.conv_sigma(x)
            z       = mu + torch.exp(0.5*sigma) * self.Normal.sample(mu.shape).to(x.device)
            
            p = self.build_prior(index)
    ...
    def forward(self, x, test_flag=False, y=0, epoch=0):
        ...
        if self.aug_methods[0]=='DaDa':
            ...
            x, kl_loss = self.data_generator(x, y, test_flag)
        else:
            kl_loss = 0
