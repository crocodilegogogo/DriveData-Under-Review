...
class DeepSense1(nn.Module):
    def __init__(self, input_2Dfeature_channel, input_channel, POS_NUM, kernel_size,
        ...
        super(DeepSense1, self).__init__()
        ...
        self.sensor_conv1 = nn.Sequential(
            nn.Conv2d(feature_channel, feature_channel, (1,2*merge_kernel_size1), (1,2), (0,0)),
            nn.BatchNorm2d(feature_channel),
            nn.ReLU(),
            # nn.MaxPool2d(2)
            )
        ...
        self.sensor_conv3 = nn.Sequential(
            ...
            nn.Conv2d(feature_channel, feature_channel, (1,2*merge_kernel_size3), (1,3), (0,0)),
            nn.BatchNorm2d(feature_channel),
            nn.ReLU(),
            # nn.MaxPool2d(2)
            )
        
        if self.datasetname in ['HAPT','HHAR','MobiAct']:
            mul = 3
    ...
    def new(self):
        network_new = DeepSense1(self.input_2Dfeature_channel, self.input_channel, self.POS_NUM, self.kernel_size,
    ...
    def my_fft_torch(self, classifier_name, tensor_list, N_intervals, len_intervals):
        ...
        if classifier_name == 'DeepSense_torch1':
