...
class Deep_Conv_LSTM(nn.Module):
    def __init__(self, input_2Dfeature_channel, input_channel, kernel_size,
        ...
        self.conv_mu         = nn.Sequential(
            nn.Conv2d(feature_channel, feature_channel, (1,kernel_size), 1, (0,kernel_size//2)),
            nn.BatchNorm2d(feature_channel, affine=False)
            )
    ...
    def data_generator(self, x, test_flag):
        ...
        if test_flag == False:
            sigma   = torch.exp(0.5*self.scaler(self.conv_sigma(x), mode='negative'))
    ...
    def scaler(self, inputs, tau=0.5, mode='positive'):
        if mode == 'positive':
            scale = tau + (1 - tau) * torch.sigmoid(inputs)
        else:
            scale = (1 - tau) * torch.sigmoid(-inputs)
